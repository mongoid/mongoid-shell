    ...
    context "a replica set" do
      ...
      it "includes username and password" do
        Mongoid::Shell::Commands::Mongorestore.new(
          session: @session,
          restore: "a folder"
        ).to_s.should == "mongorestore --host dedicated1.myapp.com:27017 --db mongoid_shell_tests --username user --password password \"a folder\""
      end
    end
    context "single host" do
      ...
      it "includes username and password" do
        Mongoid::Shell::Commands::Mongorestore.new(
          session: @session,
          restore: "a folder"
        ).to_s.should == "mongorestore --host something.mongohq.com:27017 --db mongoid_shell_tests --username user --password password \"a folder\""
      end
    end
    unless Mongoid::Shell.mongoid2?
      context "url" do
        ...
        it "includes username and password" do
          Mongoid::Shell::Commands::Mongorestore.new(
            session: @session,
            restore: "a folder"
          ).to_s.should == "mongorestore --host 59.1.22.1:27017 --db mongoid_shell_tests --username user --password password \"a folder\""
        end
      end
    end
  end
